use super::{Destination, RootBudgets, SourceFailure, SourceSnapshot, caps};

const MODIFIER_SEGMENT: i32 = 2;
...
impl DamageAllocation {
    ...
    pub(super) fn build(
        ...
        debug_assert!(
            matches!(segment, 0 | 1),
            "producer segments are validated before allocating modifier shares"
        );
        ...
        for ((total, blocked), modifier_amount) in results.iter().zip(result_budgets) {
            ...
            for (roots, amount) in roots.iter_mut().zip(portions) {
                AllocatedCredit::append(&mut credits, MODIFIER_SEGMENT, roots.take(amount)?)?;
            }
...
mod tests {
        ...
        fn build(
            ...
            for (i, (total, blocked)) in results.iter().enumerate() {
                ...
                for (j, roots) in roots.iter_mut().enumerate() {
                    ...
                    units.extend(roots.take(count).into_iter().map(|root| (root, 2)));
                }
                units.extend(
                    producer
    ...
    fn redirected_one_unit_shares_exhaust_each_supplier_once() {
        ...
        let results = [(2, 1), (2, 2)];
        let actual = DamageAllocation::build(&producer, 0, &modifiers, &results)
            .expect("four actual points exhaust two producer and two modifier roots");
        let expected = vec![
            vec![
                credit(Destination::Row(3), 2, 1, 0),
                credit(Destination::Row(1), 0, 1, 1),
            ],
            vec![
                credit(Destination::Row(2), 2, 1, 1),
                credit(Destination::Row(0), 0, 1, 1),
            ],
        ];
        assert_eq!(actual, expected);
        assert_eq!(actual, UnitModel::build(&producer, 0, &modifiers, &results));
    }
    ...
    fn overlapping_modifier_roots_coalesce_before_blocked_split() {
        ...
        assert_eq!(
            ...
            vec![
                vec![
                    credit(Destination::Row(1), 2, 2, 1),
                    credit(Destination::Row(2), 2, 1, 0),
                    credit(Destination::Row(1), 0, 1, 1),
                ],
                vec![],
            ]
    ...
    fn lethal_and_unavailable_sources_preserve_actual_totals() {
        ...
        let lethal = DamageAllocation::build(&unknown, 1, &modifiers, &[(1, 1), (0, 0)])
            .expect("modifier claims are capped by the one actual lethal point");
        assert_eq!(
            lethal,
            vec![vec![credit(Destination::Row(1), 2, 1, 1)], vec![]]
        );
        assert_eq!(
            lethal,
            UnitModel::build(&unknown, 1, &modifiers, &[(1, 1), (0, 0)])
        );
        for segment in [0, 1] {
            let results = [(0, 0), (3, 3)];
            ...
            assert_eq!(actual[1][0].segment, segment);
    ...
    fn group_limits_count_distinct_roots_and_actual_modifier_events() {
        ...
        assert!(DamageAllocation::build(&producer, 0, &modifiers, &[(2, 1), (0, 0)]).is_ok());
        modifiers.push(modifiers[0].clone());
        assert_eq!(
            DamageAllocation::build(&producer, 0, &modifiers, &[]),
            Err(SourceFailure::Capacity)
        );
        assert_eq!(
            DamageAllocation::build(&producer, 0, &[], &[(0, 0); 3]),
            Err(SourceFailure::Capacity)
        );
        let weights: Vec<_> = (0..caps::DAMAGE_DESTINATIONS).map(|row| (row, 1)).collect();
        let all_roots = source(&weights);
        assert!(DamageAllocation::build(&all_roots, 0, &[], &[(1, 0)]).is_ok());
        let extra = [ModifierContribution {
            source: source(&[(caps::DAMAGE_DESTINATIONS, 1)]),
            amount: 0,
        }];
        assert_eq!(
            DamageAllocation::build(&all_roots, 0, &extra, &[(0, 0)]),
            Err(SourceFailure::Capacity)
        );
    }
    ...
    fn malformed_groups_reject_without_partial_credits() {
        let producer = source(&[(0, 1)]);
        assert_eq!(
            DamageAllocation::build(&producer, 0, &[], &[(1, 2)]),
            Err(SourceFailure::Packet)
        );
        assert_eq!(
            DamageAllocation::build(&producer, 0, &[], &[(u64::MAX, 0), (1, 0)]),
        ...
        assert_eq!(
            DamageAllocation::build(&producer, 0, &modifiers, &[(0, 0)]),
            Err(SourceFailure::Arithmetic)
        );
        ...
        assert_eq!(
            DamageAllocation::build(&producer, 0, &stale, &[]),
    ...
    fn maximum_representable_totals_keep_root_budgets_nonnegative() {
        ...
        let actual =
            DamageAllocation::build(&producer, 1, &[], &[(u64::MAX - 1, u64::MAX - 1), (1, 0)])
                .expect("widened products preserve a representable u64 total");
        assert_eq!(
            actual,
            vec![
                vec![
                    credit(Destination::Row(0), 1, u64::MAX - 2, u64::MAX - 2),
                    credit(Destination::Row(1), 1, 1, 1),
                ],
                vec![credit(Destination::Row(0), 1, 1, 0)],
            ]
        );
    }
}
