...
use crate::data::persistence::{append_log, now_seconds, write_combat_file};
...
pub fn osty_killed(player_slot: i32) {
    let log_lines = STATE.with(|cell| {
            ...
            && let Some((id, kind)) = state.per_player[slot].active_play_source.clone()
        {
            ...
            return vec![format!(
                "  osty died: -{remaining} effective block on '{id}'\n"
            )];
        }
        state.per_player[slot].osty_stack.clear();
        vec!["  osty killed, stack cleared\n".to_owned()]
    });
    for line in log_lines {
        append_log(line);
    }
...
pub fn damage_dealt(args: DamageDealt) {
    ...
    let (log_lines, needs_osty_absorb) = STATE.with(|cell| {
        let mut state = cell.borrow_mut();
...
fn damage_dealt_in(state: &mut State, args: DamageDealt) -> (Vec<String>, bool) {
    if state
        .current
        .as_mut()
        .filter(|combat| !combat.finished)
    ...
    if args.to_player != 0 {
        let logs = record_damage_to_player_in(
            state,
            args.total,
            args.unblocked,
            args.blocked,
...
fn record_osty_dealt_in(
    ...
    state.slot_state_mut(dealer_slot).pending_contribs.clear();
    vec![format!("  osty dealt {total} damage via '{source}'\n")]
}
...
fn record_damage_to_player_in(
    ...
    if is_self_hit && unblocked > 0 {
        if let Some((index, _)) = ledger::resolve_card_in(
            ...
            logs.push(format!("  self damage {unblocked} attributed to '{id}'\n"));
        }
    } else {
        // Enemy hit: credit Strength-reduction mitigation.
        apply_str_mitigation_in(state, dealer_hash, &mut logs);
    }
    // Unlanded queue policy: the attacker's contributions drop.
    state.slot_state_mut(dealer_slot).pending_contribs.clear();
    logs.push(format!(
        "  player took {total} damage ({unblocked} unblocked)\n"
    ));
    logs
}
...
pub fn player_died(player_slot: i32) {
    let log_lines = STATE.with(|cell| {
        ...
        vec![format!("  player {player_slot} died\n")]
    });
    for line in log_lines {
        append_log(line);
    }
