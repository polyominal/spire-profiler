//! Card tests: plays (per-slot play stacks), generation (the
//! generator-tree credit model and its async-gap resolution), and
//! in-combat upgrades.
...
fn deck_and_generated_copies_of_the_same_card_credit_differently() {
    let base = scratch_dir("spire-profiler-test-gendeck");
    test_reset();
    init(&base);
...
fn generated_card_damage_credits_the_generator() {
    let base = scratch_dir("spire-profiler-test-gendmg");
    test_reset();
    init(&base);
...
fn plays_count_the_sources_own_triggers_only() {
    let base = scratch_dir("spire-profiler-test-triggers");
    test_reset();
    init(&base);
...
fn generated_card_block_credits_the_generator() {
    let base = scratch_dir("spire-profiler-test-genblk");
    test_reset();
    init(&base);
...
fn deck_card_own_id_as_explicit_source_is_a_no_op() {
    let base = scratch_dir("spire-profiler-test-deckself");
    test_reset();
    init(&base);
...
fn unrelated_explicit_source_beats_the_play_source() {
    let base = scratch_dir("spire-profiler-test-explsrc");
    test_reset();
    init(&base);
...
fn crossbow_generation_resolves_across_the_async_gap() {
    let base = scratch_dir("spire-profiler-test-genasync");
    test_reset();
    init(&base);
...
fn generation_after_the_turn_boundary_has_no_source() {
    let base = scratch_dir("spire-profiler-test-gennosrc");
    test_reset();
    init(&base);
...
#[test]
fn in_combat_upgraded_generated_card_credits_the_upgrader() {
    let base = scratch_dir("spire-profiler-test-genupg");
    test_reset();
    init(&base);
    combat_started("GEN_UPG_TEST", "test");

    card_play_started("INFERNAL_BLADE", 0, 1, 0, 0);
    card_generated(9004, "", 0, 0);
    card_play_finished(0);
    card_play_started("ARMAMENTS", 0, 1, 0, 0);
    card_upgraded(9004, 2, 2);
    card_play_finished(0);
    card_play_started("IRON_WAVE", 0, 1, 9004, 0);
    damage_dealt(DamageDealt {
        total: 7,
        unblocked: 7,
        card_source_id: "IRON_WAVE",
    ...
    assert_eq!(
        (
            armaments.damage_dealt,
            armaments.damage_dealt - armaments.damage_blocked,
            armaments.block_gained,
        ),
        (2, 2, 2)
    );
    assert_eq!(card_json(&doc, "ARMAMENTS")["dmg_upgrade"], 2);
    assert_eq!(card_json(&doc, "ARMAMENTS")["blk_upgrade"], 2);
}

#[test]
fn upgrade_split_skips_other_sources_damage() {
    let base = scratch_dir("spire-profiler-test-upgskipdmg");
    test_reset();
    init(&base);
    combat_started("UPG_SKIP_DMG_TEST", "test");
...
#[test]
fn upgrade_split_skips_proc_block() {
    let base = scratch_dir("spire-profiler-test-upgskipblk");
    test_reset();
    init(&base);
    combat_started("UPG_SKIP_BLK_TEST", "test");

    card_play_started("ARMAMENTS", 0, 1, 0, 0);
    card_upgraded(9005, 0, 3);
    card_play_finished(0);
    card_play_started("SHRUG_IT_OFF", 0, 1, 9005, 0);
    block_gained(2, "", 0, 0);
    block_gained(11, "SHRUG_IT_OFF", 0, 0);
    card_play_finished(0);
...
/// Credits exactly ONCE per play — unlike damage's per-hit rule.
#[test]
fn upgrade_block_bonus_credits_once_per_play() {
    let base = scratch_dir("spire-profiler-test-upgonce");
    test_reset();
    init(&base);
    combat_started("UPG_ONCE_TEST", "test");
    ...
    card_play_started("SHRUG_IT_OFF", 0, 1, 9005, 0);
    block_gained(11, "SHRUG_IT_OFF", 0, 0);
    block_gained(4, "SHRUG_IT_OFF", 0, 0);
    card_play_finished(0);
...
fn nested_generation_chain_collapses_to_the_root_generator() {
    let base = scratch_dir("spire-profiler-test-genchain");
    test_reset();
    init(&base);
...
fn relic_hook_generation_beats_the_ambient_play() {
    let base = scratch_dir("spire-profiler-test-genrelic");
    test_reset();
    init(&base);
...
fn potion_generated_card_credits_the_potion_kind() {
    let base = scratch_dir("spire-profiler-test-genpotion");
    test_reset();
    init(&base);
...
fn unplayed_generated_instances_contribute_nothing() {
    let base = scratch_dir("spire-profiler-test-genunplayed");
    test_reset();
    init(&base);
...
fn interleaved_plays_keep_per_slot_play_stacks() {
    let base = scratch_dir("spire-profiler-test-mp-interleave");
    test_reset();
    init(&base);
...
fn same_owner_plays_produce_per_slot_rows() {
    let base = scratch_dir("spire-profiler-test-mp2-sameowner");
    test_reset();
    init(&base);
...
fn generated_card_play_credits_the_generators_slot() {
    let base = scratch_dir("spire-profiler-test-mp2-genslot");
    test_reset();
    init(&base);
