...
# This is a vendored copy of Microsoft's official .NET SDK installer script.
#
#   Source:     https://dot.net/v1/dotnet-install.sh (canonical redirect to
#               https://builds.dotnet.microsoft.com/dotnet/scripts/v1/dotnet-install.sh)
#   License:    MIT (the .NET install-scripts repository, dotnet/install-scripts)
#   Vendored:   2026-08-13
#   Upstream SHA-256 of the retrieved file:
#               082f7685e156738a1b2e2ed8381a621870d4ce8e8c59278034556f05c186eb2e
#
# Local modification (one line): the default channel is pinned to 9.0.
# Upstream defaults to `LTS`, which in 2026 would install an SDK 10.x —
# whose targeting packs do not cover the generated shim's net9.0 target
# without a NuGet restore. An SDK 9.0.x bundles the net9.0 targeting pack
# (Microsoft.NETCore.App.Ref 9.0.x), so the net9.0 shim builds with no
# NuGet restore at all (offline after the one SDK download). The xtask
# always passes --channel explicitly; this default is the manual-use
# fallback. Everything else is byte-identical to upstream.
# ───────────────────────────────────────────────────────────────────────────
