...
- `SPIRE_PROFILER_DATA_DIR` overrides the data root. The [statistics
  store](../profiler-store/src/lib.rs) owns the database format and version;
  [storage inspection](interop.md#storage-inspection) describes legacy import
  and collecting files for diagnosis.
