...
internal abstract class GameAttributionBackend : AttributionBackend
{
    ...
    {
        CardModel card => new(CaptureKind.CardInstance, ProducerRole.Card, card.Id?.Entry ?? "", 0, SpireProfilerMod.CreditorSlot(card.Owner), Combat: card.Owner?.Creature?.CombatState),
        ...
        RelicModel relic => new(CaptureKind.DirectModel, ProducerRole.Relic, relic.Id?.Entry ?? "", 1, SpireProfilerMod.CreditorSlot(relic.Owner), Combat: relic.Owner?.Creature?.CombatState),
        PotionModel potion => new(CaptureKind.DirectModel, ProducerRole.Potion, potion.Id?.Entry ?? "", 3, SpireProfilerMod.CreditorSlot(potion.Owner), Combat: potion.Owner?.Creature?.CombatState),
        OrbModel orb => new(CaptureKind.OrbInstance, ProducerRole.Orb, orb.Id?.Entry ?? "", 5, SpireProfilerMod.PlayerSlot(orb.Owner), Combat: orb.Owner?.Creature?.CombatState),
    ...
    internal override CreatureDescriptor DescribeCreature(object model)
    {
        ...
        return new(creature.IsPlayer, creature.Monster is MegaCrit.Sts2.Core.Models.Monsters.Osty,
            creature.IsPlayer ? SpireProfilerMod.PlayerSlot(creature.Player) : creature.PetOwner != null ? SpireProfilerMod.PlayerSlot(creature.PetOwner) : 4, creature.CombatState);
    }
    internal override PowerObservation ObservePower(object model, object owner = null)
    {
        ...
        int slot = creature?.IsPlayer == true ? SpireProfilerMod.PlayerSlot(creature.Player) : creature?.PetOwner != null ? SpireProfilerMod.PlayerSlot(creature.PetOwner) : 4;
