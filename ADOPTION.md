...
Checkpoint: `tooltip` test extraction, based on clean HEAD `2736ec0cb402a76616ce31b29de2d24df63c39e5` on 2026-09-09. The `run_layout` extraction is committed there. Implementation, exact source parity, independent review (SHIP), and required checks are complete. Next: `snapshot` test extraction.
The user authorizes the root orchestrator to commit each Mechanical extraction item after implementation, independent review, gates, and checkpoint update. This exception covers `run_layout`, `tooltip`, and `snapshot`; subagents never commit. Stop after the section is complete, and resume the human-only commit rule for later work. The two `run_layout` em-dash comment edits are approved; remaining extractions preserve wording. Stop and discuss instructions that need clarification.
...
- `cargo nextest run --package profiler_core --locked --no-fail-fast --filter-expr 'test(ui::tooltip::tests::)'` passed all 20 tests under unchanged module paths. They retain palette matching, text wrapping/truncation, viewport clamping/flipping, tip stacking/fallback, and control-frame bounds.
- Exact comparison against the committed original verified an unchanged production prefix and a test body matching the independently dedented and rustfmt-formatted original. All names, fixture data, helpers, and assertions remain intact.
- `cargo xtask smoke` passed 362/362 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.4% (3,062 / 29,327); em-dash pins remain 38 files / 128 dashes. `cargo fmt --all`, `cargo xtask fmt-md`, and `git diff --check` passed. This extraction requires no new build or headless run; prior runtime/platform limits below still apply.
...
Each concern is move-only, preserving names, fixtures, module paths, and snapshots.
