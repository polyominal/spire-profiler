...
Uncommitted checkpoint: consistent signed creature hashes, based on HEAD `0f5ada92931a1b66d8f84f5326a515153bd24944` on 2026-09-09. The previous catalog-fixture checkpoint is committed there.
Implementation, independent review (SHIP), and required checks are complete. Stop for the human commit before the next item: replace the source-kind round-trip test with stored-record decoding coverage for 3/4/255.

Source audit: installed game and decompiled provenance both match `v0.111.0`. In the decompiled snapshot, `Creature` inherits `Object` without a `GetHashCode` override; `PowerModel.Owner` holds a `Creature` and rejects owner changes. `PowerModel.SetAmount`, `DoomPower.DoomKill`, `CombatHistory.DamageReceived`, and `Hook.AfterDamageGiven` operate on creature references. These support retaining the existing hash identity; the game's separate nullable `CombatId` is not substituted. This is a consistency simplification with no demonstrated runtime bug.
...
- `cargo xtask smoke` passed 363/363 tests, workspace Clippy, private rustdoc, formatting, citations, ABI, and em-dash checks. The count drops by one from HEAD because the cast-only test is gone. Rust comment density is 10.4% (3,077 / 29,481); em-dash pins remain 39 files / 130 dashes. `cargo xtask check-abi` separately verified all 38 bindings. `cargo fmt --all`, `cargo xtask fmt-md`, and `git diff --check` passed.
- Targeted `cargo test --locked --package profiler_core --lib` runs passed `data::events::tests::power` (9/9), `data::events::tests::combat` (15/15), and `data::ledger::tests` (15/15). `cargo test --locked --package profiler_core --test sim` passed 3/3. Source audit found no widened creature-hash declarations/casts or helper references remaining; the ABI checker's unrelated generic `u64` fixture is retained.
...
- `ZIG_GLOBAL_CACHE_DIR="$PWD/target/zig-cache" cargo xtask build` built all four native targets and the C# shim; C# had zero warnings/errors. Native cross-compilation emitted the existing macOS SDK-discovery and deprecated-linker-setting warnings but completed. With the same override, `cargo xtask headless-test` installed the matched bundle and passed against Windows game v0.111.0 through WSL: game exit status 0 after 39.7 seconds, 194 patches (minimum 194), all draw/persistence markers, no unexpected profiler errors, command exit 0. WSL interop and installation into the game directory required sandbox escalation.
