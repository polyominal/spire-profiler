...
Checkpoint: simulation assertion cleanup, based on clean HEAD `3d452f3d5d48e3533bd4ffceb3c0d463e1235b8c` on 2026-09-09. The ABI empty-string item is committed there. Implementation, independent review (SHIP), and required checks are complete. Next: run-accumulator vector equality.
...
- `cargo nextest run --package profiler_core --test sim --locked --no-fail-fast` and `SIM_SEED=0 cargo nextest run --package profiler_core --test sim --locked --no-fail-fast` each passed all 3 simulations. They retain failures for ledger/queue invariants, independent context/block models, missing or extra combat records, parse/live mismatches, and combat/run outcomes.
- `cargo nextest run --package profiler_core --locked --no-fail-fast --filter-expr 'test(build_combat_json_matches_the_documented_schema) | test(all_zero_card_rows_round_trip_as_identity_only) | test(build_run_json_matches_the_documented_schema)'` passed all 3 serialization tests with unchanged snapshots.
- `cargo xtask smoke` passed 363/363 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.4% (3,064 / 29,370); em-dash pins remain 39 files / 130 dashes. `cargo xtask fmt-md` and `git diff --check` passed. This test-only change requires no new build or headless run; prior runtime/platform limits below still apply.
