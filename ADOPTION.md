...
Uncommitted checkpoint: catalog fixture isolation, based on clean HEAD `b823b50898bc6d79986742a724758f9951e0f598` on 2026-09-09. The previous ownership/reproducibility-policy checkpoint is committed there.
Implementation, independent review (SHIP), and required checks are complete. Stop for the human commit before the next item: strengthen signed doom-hash attribution coverage before replacing the cast-only test.
...
- The three filesystem-backed tests in [check_catalog.rs](xtask/src/check_catalog.rs) hold owned `xshell::TempDir` fixtures through their assertions. Fixed shared paths and manual teardown are removed; fixture bytes, test names, hook discovery, namespace-drift rejection, and bodyless-declaration rejection are preserved. The source diff adds 17 lines and removes 11, with no new tests, helpers, dependencies, platform conditions, or production changes. Headless/decompile scratch is unchanged.

Verification:

- `cargo xtask smoke` passed 364/364 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.4% (3,078 / 29,455); em-dash pins remain 39 files / 130 dashes. `cargo fmt --all`, `cargo xtask fmt-md`, and `git diff --check` passed. The test-fixture-only change requires no new headless or live catalog run.
- `cargo test --locked --package xtask check_catalog::tests` passed all 10 catalog tests. After `cargo test --locked --package xtask --no-run --message-format=json` located the test executable, four concurrent processes each ran the three changed tests with `--exact` and `--test-threads=3` against one isolated shared `TMPDIR`: all 12 executions passed. Sentinels at the three former fixed paths and occupied `xshell-tmp-dir-0`/`xshell-tmp-dir-1` directory/file candidates retained their bytes; all newly owned fixture directories were removed. The probe did not overlap full suites.
