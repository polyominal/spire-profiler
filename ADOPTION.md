...
Checkpoint: `unreachable_pub` visibility trial, based on clean HEAD `6d05d499a1ba84ea65ec84f384946109686ae657` on 2026-09-09. Implementation, independent review (SHIP), source comparison, focused checks, and smoke are complete. Ready for the human commit; stop before the separate safe-scalar-export and ABI-scanner item.
The user authorizes this bounded visibility trial under the implementation/review workflow. The Mechanical extraction commit exception has ended; all commits and PRs are human-owned. Stop and discuss substantive instruction ambiguities.
...
- `cargo check --package profiler_core --locked --no-default-features` and `cargo clippy --package profiler_core --all-targets --all-features --locked -- -D warnings` passed, covering production and test-support configurations. `cargo fmt --all -- --check` and `git diff --check` passed.
- Source comparison confirms only seven visibility modifiers and the lint attribute change. Function bodies, tests, comments, C export declarations, containment, registration callbacks, persisted schema, and snapshots are unchanged.
- `cargo xtask smoke` passed 362/362 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.4% (3,062 / 29,327); em-dash pins remain 38 files / 128 dashes. `cargo xtask fmt-md` and `git diff --check` passed. This visibility-only change requires no new headless run; prior runtime/platform limits below still apply.
