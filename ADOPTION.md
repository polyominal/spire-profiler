...
Checkpoint: safe scalar C exports and ABI-scanner support, based on clean HEAD `39e0b8fffe853cd3e2efed92c6429764605260dc` on 2026-09-09. Implementation, independent review (SHIP), source comparison, focused tests, smoke, check-abi, the full build, and headless are complete. Ready for the human commit; stop before xtask simplification.
The user authorizes this bounded ABI item under the implementation/review workflow. All commits and PRs are human-owned. Stop and discuss substantive instruction ambiguities.
...
- `cargo test --workspace --locked abi::` passed all eight core ABI and nine scanner tests. The mixed fixture verifies two bindings; both safe and unsafe parameter-order drift and missing exports produce the expected errors. Null/empty/non-UTF-8 fallback, context overflow, persistence, containment, scroll routing/recovery, and panel/history behavior remain covered.
- Source comparison verifies unchanged production code apart from export safety qualifiers and necessary comments, including all 39 C parameter lists and bodies. All eight ABI test names, fixtures, calls, and assertions remain; only unnecessary unsafe scopes/comments are removed.
- `cargo xtask smoke` passed 362/362 tests, workspace Clippy, private rustdoc, formatting, citations, ABI, and em-dash checks. Separate `cargo xtask check-abi` verified all 38 shim bindings. Rust comment density is 10.3% (3,005 / 29,279); em-dash pins are 38 files / 126 dashes. `cargo fmt --all -- --check`, `cargo xtask fmt-md`, and `git diff --check` passed.
- `ZIG_GLOBAL_CACHE_DIR="$PWD/target/zig-cache" cargo xtask build` passed all four native targets and the C# shim (zero C# warnings/errors), using the installed pinned game v0.111.0. The bundle contains this uncommitted ABI change and records its base commit `39e0b8ff`. Existing macOS SDK-discovery and deprecated-linker-setting warnings remain nonfatal.
...
- `ZIG_GLOBAL_CACHE_DIR="$PWD/target/zig-cache" cargo xtask headless-test` installed the matched bundle containing this ABI change and passed against Windows game v0.111.0 through WSL: game exit status 0 after 39.4 seconds, 194 patches (minimum 194), all draw/persistence markers, no unexpected profiler errors, command exit 0. WSL interop and installation into the game directory required sandbox escalation.
