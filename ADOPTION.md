...
Uncommitted checkpoint: stored-record source-kind decoding coverage, based on clean HEAD `3fa56dfcfa3918d009552f56aaf1b3ba4a424f3d` on 2026-09-09. Initial Git status was clean; the Zig resolver checkpoint is committed there.
Implementation, independent review (SHIP), and required checks are complete. Stop for the human commit before the next item: preserve explicit wire decoder cases, remove the membership-only source-kind sweep, and correct the source-kind clamp documentation/diagnostic and modifier-kind comment without changing behavior.
...
- [Record tests](profiler-core/src/data/records/tests.rs) replace the ledger's `from_u8_round_trips_all_kinds` with literal JSON parsed through `parse_combat_doc`: 3 reads as Potion, 4 as Osty, and 255 falls back to Osty. This catches incorrect stored-kind mappings, accidental context-kind clamping, and unknown-kind rejection or wrong fallback. The identity-only Potion fixture and surrounding tests remain intact; production behavior, ABI, schema, and snapshots are unchanged. Rust context is 10 lines added, 8 deleted, with no moves or helpers.

Verification:

- `cargo nextest run --package profiler_core --locked --no-fail-fast --filter-expr 'test(data::records::tests::) | test(data::persistence::combat_doc::tests::) | test(data::ledger::tests::)'` passed all 27 targeted tests, including the retained identity-only fixture and persistence snapshots.
- `cargo xtask smoke` passed 363/363 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.4% (3,071 / 29,469); em-dash pins remain 39 files / 130 dashes. `cargo fmt --all -- --check`, `cargo xtask fmt-md`, and `git diff --check` passed. This test-only change requires no new build or headless run; prior runtime/platform limits below still apply.
