...
Checkpoint: `run_layout` test extraction, based on clean HEAD `691fe7438ce717c54fb6c42de02b50830ce1976f` on 2026-09-09. Implementation, source/snapshot parity checks, independent review (SHIP), and required checks are complete. Next: `tooltip` test extraction.
...
- `cargo nextest run --package profiler_core --locked --no-fail-fast --filter-expr 'test(ui::run_layout::tests::)'` passed all 14 tests under unchanged module paths. They retain identity/roster/icon fallback, layout bounds/heights, header/body placement, hit routing, full-list rendering, and four command snapshots.
- An exact source comparison against HEAD verified the production prefix and dedented test body, allowing only the two approved comment edits, relocated imports, module declaration change, and two required rustfmt adjustments (merged imports and a collapsed call). A separate Git `diff --no-index --find-renames=100% --name-status` over isolated original/final snapshot trees reported four R100 renames, including unchanged snapshot metadata; no snapshot updates were accepted.
- `cargo xtask smoke` passed 362/362 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.4% (3,062 / 29,329); em-dash pins dropped to 38 files / 128 dashes. `cargo fmt --all`, `cargo xtask fmt-md`, and `git diff --check` passed. This extraction requires no new build or headless run; prior runtime/platform limits below still apply.
