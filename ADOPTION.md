...
Checkpoint: Mechanical extraction complete, with the final `snapshot` change based on clean HEAD `161841cb6b30864bacce3901df8fa2a6513b9591` on 2026-09-09. All three items passed independent review (SHIP), source parity checks, targeted tests, and smoke; all 45 test names are preserved. Stop after this commit, before the API and ABI work.
The user's root-only commit exception covers the three Mechanical extraction items after implementation, independent review, gates, and checkpoint update. It ends with this `snapshot` commit; later work follows the human-only commit rule. Subagents never commit. Only the two approved `run_layout` comments change wording. Stop and discuss instructions that need clarification.
...
- `cargo nextest run --package profiler_core --locked --no-fail-fast --filter-expr 'test(ui::snapshot::tests::)'` passed all 11 tests under unchanged module paths. They retain row ordering/caps, player filters, self-damage and defense ranking, run/footer totals, arbitrary-dataset handling, and detail attribution.
- Exact comparison against the committed original verified an unchanged production prefix and a byte-identical test body after dedentation. All names, fixture data, state setup/reset, helpers, and assertions remain intact.
- `cargo xtask smoke` passed 362/362 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.4% (3,062 / 29,326); em-dash pins remain 38 files / 128 dashes. `cargo fmt --all`, `cargo xtask fmt-md`, and `git diff --check` passed. These extractions require no new build or headless run; prior runtime/platform limits below still apply.
