...
Uncommitted checkpoint: ownership and reproducibility policy, based on clean HEAD `6c21a577dddcda1477cd2d5396691e0614ccba12` on 2026-09-09. The previous outer-Cargo-locking checkpoint is committed there.
Implementation, independent review (SHIP), and required checks are complete. Stop for the human commit before the next item: isolate the three catalog test fixtures in [check_catalog.rs](xtask/src/check_catalog.rs).
...
- Ownership policy now points to [state.rs](profiler-core/src/data/state.rs) for live combat/run data and borrowing, and to the [log sink](profiler-core/src/data/persistence/log.rs) for no-`STATE`-reentry. Independent owners retain their lifetimes; gameplay-state coordination alone excludes locks/atomics. Owned test/xtask scratch is permitted, with fixed headless/decompile paths requiring serialized invocations. [sim.rs](profiler-core/tests/sim.rs) distinguishes seeded behavioral replay from wall-clock timestamps; [AGENTS.md](AGENTS.md) retains the separate reviewed byte-snapshot policy. Six documentation files changed, 41 lines added and 40 removed before this handoff; duplicated ownership prose was deleted. No runtime code, test logic, snapshots, dependencies, or platform support changed.

Verification:

- `cargo xtask smoke` passed 364/364 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.5% (3,078 / 29,449); em-dash pins remain 39 files / 130 dashes. `cargo fmt --all`, `cargo xtask fmt-md`, and `git diff --check` passed. The documentation-only change requires no new headless run.
