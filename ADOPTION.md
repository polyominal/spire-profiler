...
Uncommitted checkpoint: locked outer Cargo invocation, based on clean HEAD `7cd810d4664fbdb88a39a11f5c412ca5634ce41e` on 2026-09-09. The previous headless-verdict checkpoint is committed there.
Implementation, independent review (SHIP), and required checks are complete. Stop for the human commit before the next correction, reconciling ownership and reproducibility policy.

Completed work:

- Smoke covers ABI, private rustdoc with warnings denied, comment density, and documentation hygiene. The outer alias in [.cargo/config.toml](.cargo/config.toml) and inner Cargo gate/build calls use `--locked`. The alias change replaces one line, preserving package selection and argument forwarding; no source code or permanent tests are added. Nextest uses `--no-fail-fast`; production printing/unwrap lints and the em-dash ratchet are installed. Canonical documentation ownership, trims, soft module-doc budgets, and the shared line scanner landed.
...
- An isolated system-temp workspace copied the repository alias and pinned toolchain, with an `xtask` binary depending only on a local crate. After `cargo generate-lockfile --offline`, changing the dependency version from 0.1.0 to 0.2.0 made the lock stale. With `CARGO_NET_OFFLINE=true`, `cargo xtask probe` exited 101 with an explicit `--locked` refusal before compilation/execution, preserving lockfile bytes, inode, size, mtime, and ctime. The original alias from HEAD updated that same stale lock and ran the binary. Restoring the new alias then ran successfully with the repaired lock, forwarded `probe`, and preserved the lock fingerprint. No network or repository lockfile mutation was needed.
- The repository `Cargo.lock` SHA-256 remains `793c921c9ba1a75df2dbc37926de3351301081bc6b910ac19ca1584bac68568a`; inode, size, mtime, and ctime also match the pre-probe values. `cargo xtask --help`, `cargo xtask fmt-md`, and `git diff --check` passed. The alias-only change requires no new headless run.
- `ZIG_GLOBAL_CACHE_DIR="$PWD/target/zig-cache" cargo xtask headless-test` passed for the headless-verdict implementation committed in `7cd810d`, against Windows game v0.111.0 through WSL: game exit status 0 after 36.3 seconds, 194 patches (minimum 194), all draw/persistence markers, no unexpected profiler errors, command exit 0. All four native targets and the C# shim built; C# had zero warnings/errors. Native cross-compilation emitted macOS SDK-discovery and deprecated-linker-setting warnings but completed. WSL interop and installation into the game directory required sandbox escalation.
...
### Reconcile ownership and reproducibility policy
