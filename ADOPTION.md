...
Uncommitted checkpoint: Zig resolver simplification, based on clean HEAD `9471d34b23b664a3a413726d9a3ff97846eaefb7` on 2026-09-09. The previous creature-hash checkpoint is committed there. The user selected this bounded item ahead of the default queue.
...
- `cargo xtask smoke` passed 363/363 tests, workspace Clippy, private rustdoc, formatting, citations, 38 ABI bindings, and em-dash checks. Rust comment density is 10.4% (3,072 / 29,467); em-dash pins remain 39 files / 130 dashes. `cargo fmt --all`, `cargo xtask fmt-md`, and `git diff --check` passed.
- `zig-sdk/zig version` reported the pinned 0.16.0. `ZIG_GLOBAL_CACHE_DIR="$PWD/target/zig-cache" cargo xtask build` exercised the simplified resolver with the cached SDK and built all four native targets and the C# shim; C# had zero warnings/errors. The existing macOS SDK-discovery and deprecated-linker-setting warnings were nonfatal. Missing/wrong-version refresh and download-failure paths were reviewed in the unchanged installer, not executed against the real SDK. This build-tool-only change requires no new headless run.
